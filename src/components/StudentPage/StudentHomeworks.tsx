...
import { useStudentHomework, useTextHelpers } from "@/store";

export default function StudentHomeworks() {
  const { studentHomework } = useStudentHomework();

  const { monthNumberToName, toArabicDigits } = useTextHelpers();
  return (
      ...
      >
        {studentHomework.map((el) => (
          ...
          >
            <div className="flex justify-center items-center gap-1">
              <div className="w-12 h-12 flex flex-col bg-white justify-center items-center rounded-lg">
                <h1 className="text-second-texty-color text-[10px] font-bold">
                  {monthNumberToName(el.dueDate)}
                </h1>
                <h2 className="text-main-color text-sm font-black">
                  {toArabicDigits(el.dueDate)}
                </h2>
              </div>
            ...
            >
              {el.attachments.length > 0 ? "تم التسليم" : "لم يسلم"}
            </div>
          </div>
        ))}
      </div>
